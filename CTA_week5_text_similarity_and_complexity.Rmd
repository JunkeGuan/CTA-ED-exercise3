...
dim(dfmat_grouped)
```
#24 = 24 Cabinet MPs; 26879 = features

```{r}
euclid_dist <- textstat_dist(dfmat_grouped, 
                             margin = "documents", 
                             method = "euclidean")
as.matrix(euclid_dist)[1:5, 1:5]
```
#This computes pairwise Euclidean distances between the aggregated tweets of each MP. Smaller values indicate more similar word usage patterns, while larger values suggest greater linguistic divergence.

```{r}
rownames(as.matrix(euclid_dist))
```
```{r}
may_euclid <- as.data.frame(as.matrix(euclid_dist)[23, c(1:22, 24)])

colnames(may_euclid) <- "euclid_dist"

may_euclid <- tibble::rownames_to_column(may_euclid, "username")

may_euclid
```
#Note#
To address Exercise 1, I first grouped the document-feature matrix by username so that all tweets from each MP were aggregated into a single document. I then computed Euclidean distances between these aggregated documents using `textstat_dist()`.  
Next, I extracted the row corresponding to Theresa May to examine how linguistically similar each MP’s tweets are to those of the Prime Minister. Smaller Euclidean distance values indicate more similar word usage patterns, while larger values suggest greater divergence in language use.
...
```{r}
textstat_readability(speeches$text)
```
#Flesch = Flesch Reading Ease；higher = easier to read
```{r}
readability_all <- textstat_readability(
  speeches$text,
  measure = c("Flesch", "SMOG", "FOG")
)

head(readability_all)
```
```{r}
speeches$Flesch <- readability_all$Flesch
speeches$SMOG   <- readability_all$SMOG
speeches$FOG    <- readability_all$FOG

head(speeches)
```
```{r}
complexity_summary <- speeches %>%
  group_by(speaker) %>%
  summarise(
    mean_Flesch = mean(Flesch, na.rm = TRUE),
    mean_SMOG   = mean(SMOG, na.rm = TRUE),
    mean_FOG    = mean(FOG, na.rm = TRUE),
    N = n()
  )
...
```
#Note#
To extend the analysis beyond the Flesch–Kincaid score, I estimated three additional readability measures: Flesch, SMOG, and FOG. These indicators capture different aspects of textual complexity, where higher Flesch scores indicate easier readability, while higher SMOG and FOG scores suggest greater linguistic complexity.
The results show a consistent pattern across the four speakers. David Cameron has the highest Flesch score and the lowest SMOG and FOG values, indicating that his speeches are generally easier to read and less complex. In contrast, J.L.R. Zapatero shows the lowest Flesch score and the highest SMOG and FOG values, suggesting a more complex and demanding rhetorical style. Gordon Brown and Mariano Rajoy fall in between these two extremes, displaying moderate levels of complexity across all three measures.
Overall, the comparison across readability metrics produces a substantively similar ranking of speakers. This suggests that the findings are robust to the choice of complexity measure and supports the interpretation that Cameron’s speeches are linguistically simpler, while Zapatero’s are comparatively more complex.
...
```{r}
tweets$week <- format(as.Date(tweets$date), "%Y-%U")
head(tweets)
```

```{r}
head(tweets[, c("date", "week")])
```
```{r}
tweets_weekly <- tweets %>%
  group_by(username, week) %>%
  summarise(text = paste(tweet, collapse = " "), .groups = "drop")

head(tweets_weekly)
#For each week, all the tweets of each MP are combined and then the similarity with Theresa May is calculated.
```
```{r}
tweets_weekly_corpus <- corpus(
  tweets_weekly,
  text_field = "text"
)

docvars(tweets_weekly_corpus, "username") <- tweets_weekly$username
docvars(tweets_weekly_corpus, "week") <- tweets_weekly$week

tweets_weekly_corpus
```
#686 documents = MP × week；2 docvars = username & week；each document = all tweets of one certain MP in one week
...
```{r}
dfmat_weekly <- dfm(toks_weekly)
dfmat_weekly
```
```{r}
docnames(dfmat_weekly) <- paste(tweets_weekly$username,
                                tweets_weekly$week,
                                sep = "_")
head(docnames(dfmat_weekly))
```
```{r}
cos_weekly <- textstat_simil(
  dfmat_weekly,
  margin = "documents",
  method = "cosine"
)
...
```
```{r}
cos_df <- as.data.frame(as.matrix(cos_weekly))
cos_df$doc1 <- rownames(cos_df)
...
head(cos_long)
#I converted the cosine similarity matrix into a long-format dataframe so that each row represents the similarity between two MP-week documents. This structure is easier for visualisation
```
```{r}
library(dplyr)
library(tidyr)
...
cos_long2 <- cos_long %>%
  separate(doc1, into = c("user1", "week"), sep = "_") %>%
  separate(doc2, into = c("user2", "week2"), sep = "_")

head(cos_long2)
#I separated the document identifiers into username and week components in order to match MP tweets with PM tweets within the same week
```
```{r}
cos_sameweek <- cos_long2 %>%
  filter(week == week2)

head(cos_sameweek)
```
```{r}
unique(cos_sameweek$user1)
```
```{r}
library(ggplot2)

ggplot(cos_sameweek, aes(x = week, y = similarity)) +
  geom_boxplot() +
  labs(
    title = "Similarity of MP tweets within the same week",
    x = "Week",
    y = "Cosine similarity"
  ) +
  theme_minimal()
```
#Note#
The boxplot shows the distribution of cosine similarity scores between MPs’ tweets within the same week. Overall, similarity scores are relatively low, with most values concentrated below 0.2. This suggests that MPs tend to discuss diverse topics even within the same week, indicating a high degree of individualised communication strategies. 

However, occasional higher similarity values (around 0.3–0.6) suggest that MPs sometimes respond to shared political events or common policy agendas. The consistent presence of values at 1 reflects comparisons of each document with itself, which is expected in similarity matrices. 

Overall, the results indicate that while there is some convergence in political messaging, MPs’ weekly tweets are generally heterogeneous in content.
